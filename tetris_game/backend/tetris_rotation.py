 ...
 def rotate(self):
    ...
    def rotate(self, xc, yc):
        ...
        for x, y in self.points:
            x1 = xc + (x - xc) * cos(-pi / 2) - (y - yc) * sin(-pi / 2)
            y1 = yc + (x - xc) * sin(-pi / 2) + (y - yc) * cos(-pi / 2)
            new_points.append((round(x1), round(y1)))
...
class Cell:
    ...
    def rotate(self, xc, yc):
        ...
        for x, y in self.points:
            ...
            new_points.append((round(x1), round(y1)))
        self.points = new_points
        """rotate all points in cell by -pi/2 around given center: (xc, yc)
        fmla: x' = xc + (x-xc)*cos(-pi/2) - (y-yc)*sin(-pi/2)
